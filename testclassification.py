...
print('Number of stars with classifications:',classedsample)

# check in a range of probabilities to see how we did
lowbound,highbound = 0.9,1.0
barmems = ((np.nanmedian(allprobs[:,:,barcomp],axis=0)>lowbound) & (np.nanmedian(allprobs[:,:,barcomp],axis=0)<highbound) & (Stars['bulge'][criteria]>0.)& (Stars['bulge'][criteria]>=0))
print('How many stars classed as bar and are bar?')
print(Stars['x'][criteria][barmems].size) # particles that are a part of the bar but not classed as such (WRONG)
barmems = ((np.nanmedian(allprobs[:,:,barcomp],axis=0)>lowbound) & (np.nanmedian(allprobs[:,:,barcomp],axis=0)<highbound))
print('How many stars in {}-{} bin?'.format(lowbound,highbound))
print(Stars['x'][criteria][barmems].size) # particles that are a part of the bar but not classed as such (WRONG)
