...
data = pd.read_csv('data/apogee/fits/table_independent1.csv')
...
# this method is subject to measurement uncertainty at 20 samples
...
for indx, i in tqdm.tqdm_notebook(enumerate(weights.T)):
    def loss2(w):
        XS,YS = (R+np.random.normal(0.,eR))**2,Lz+np.random.normal(0.,eLz)
